...
# to avoid limits
default_client_id=923c5fe87d5bae3
...
while [ $# -gt 0 ]; do
	...
	#clip+="[url=""$url""][img]"
	#"[url=" "$song_full" "][img]" "$song_full_thumb" "[/img][/url]"
...
if type pbcopy &>/dev/null; then
	printf "[b]Spectrograms:[/b]\n\n[url="${clip[1]}"][img]"${clip[2]}"[/img][/url]\n\n[url="${clip[3]}"][img]"${clip[4]}"[/img][/url]\n" | pbcopy
elif [ $DISPLAY ]; then
	if type xsel &>/dev/null; then
		echo "$clip" | xsel -i
	elif type xclip &>/dev/null; then
